...
func Init(cfg utils.GlobalCfg, wg *sync.WaitGroup, sigc chan bool) error {
	...
	// 如果docker-compose.yml中配置了LOG_FILEPATH和LOG_FILENAME，记录日志到文件中。但是日志没有限制大小，导致1天的日志文件会非常大。
	// 如果没有配置LOG_FILEPATH和LOG_FILENAME，记录日志到标准输出。
	...
	if cfg.MongoEnable {
		...
		if err != nil {
			...
			if err != nil {
				...
				// collectionName := strings.Join([]string{cfg.MongoCollection, formattedDate}, "_")
				// gCollection = gMongo.Database(cfg.MongoDB).Collection(collectionName)
				// fmt.Println("=============gCollection", gCollection)
				// timerChangeMongoCollection(cfg, gMongo, sigc, wg)
			}
		}
		...
		for i := 0; i < cfg.MongoWorkerNum; i++ {
			...
			go utils.MongoWorker(gMongo, wg, sigc, cfg.MongoDB, cfg.MongoCollection)
		}
	}
	...
	// 初始化客户ID黑名单
	blacklistConfig := blacklist.InitConfigBlacklist{BlacklistFilePath: cfg.BlacklistConfigName}
...
func PreHandle() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			fmt.Println("读取requestbody失败：", err)
		} else {
			// fmt.Println("请求的body是:", string(reqBody))

			// 重置请求主体流的位置
			c.Request.Body = io.NopCloser(bytes.NewReader(reqBody))
			traceLogObj.OrgReqeustBody = string(reqBody)
		}
...
func ProxyHandle() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		proxy.ModifyResponse = func(resp *http.Response) error {
			...
			if err != nil {
				fmt.Println("读取responsebody失败：", err)
			} else {
				// fmt.Println("响应的body是:", string(resBody))

				// 重置请求主体流的位置
				resp.Body = io.NopCloser(bytes.NewReader(resBody))
				traceLogObj.OrgResponseBody = string(resBody)
			}
...
// 定时切换MongoDB的集合
// 入参：cfg：全局配置变量；client：连接MongoDB的client；sigc：与MongoWorker交互通道；wg：sync.WaitGroup
// 返回值：无
func timerChangeMongoCollection(cfg utils.GlobalCfg, client *mongo.Client, sigc chan bool, wg *sync.WaitGroup) {

	// 计算下一个00:00:01的时间
	nextTime := time.Date(time.Now().Year(), time.Now().Month(), time.Now().Day(), time.Now().Hour(), time.Now().Minute()+10, time.Now().Second(), 0, time.Local)
	...
	go func() {
		...
		if client != nil {
			...
			collectionName := strings.Join([]string{cfg.MongoCollection, formattedDate}, "_")
			gCollection = client.Database(cfg.MongoDB).Collection(collectionName)
			fmt.Println("gCollection", gCollection)
		} else {
		...
		// 停止当前计时器，启动下一次定时器
		timerChangeMongoCollection(cfg, client, sigc, wg)
	}()
}
